...
- **Nhận diện giọng nói: ưu tiên Cloudflare Worker proxy Groq Whisper API (`large-v3-turbo`), tự động DỰ PHÒNG sang Whisper (model `tiny.en`) chạy client-side qua `@huggingface/transformers` (v3) nếu Worker chưa cấu hình/gọi lỗi** (xem `src/lib/pronunciationApi.js`). Đây là NGOẠI LỆ BaaS thứ 2 tương tự Firebase/Cloudinary — **Groq free tier (~8 giờ audio/ngày, 20 request/phút) không cần thẻ tín dụng**, đủ dùng cho quy mô ~100 học sinh (ước tính ~100 giờ/tháng ≈ 3,3 giờ/ngày, dưới mức free). Worker code ở `worker/` (deploy độc lập bằng `wrangler`, xem `worker/README.md`), cấu hình qua `VITE_WORKER_URL` (`.env.example`, GitHub Actions secret). **Khác thiết kế Azure trước đây (đã bỏ): có fallback** — người dùng chủ động yêu cầu giữ Whisper local làm dự phòng khi build lại tính năng gọi API ngoài, để không phụ thuộc hoàn toàn dịch vụ bên thứ 3 (mất mạng/hết quota Groq trong ngày vẫn dùng được mic, chỉ giảm độ chính xác tạm thời). Không tự ý bỏ nhánh dự phòng Whisper local khi chưa hỏi lại người dùng.
  Nhánh Whisper local (dự phòng) — đã thử `small.en` rồi `base.en` (chính xác hơn `tiny.en`) nhưng gặp liên tiếp 2 bug đã biết của thư viện khi decoder dùng dtype q8 ("quantized"): WebGPU + decoder q8 cho kết quả sai/gibberish ([issue #1317](https://github.com/huggingface/transformers.js/issues/1317)), WASM + decoder q8 crash lúc tạo session với lỗi `Missing required scale... MatMulNBits` ([issue #1707](https://github.com/huggingface/transformers.js/issues/1707), test thực tế gặp y hệt lỗi này với `base.en`). Cấu hình DUY NHẤT chạy đúng ổn định: **decoder dùng dtype `q4`** thay vì `q8` — nhưng decoder q4 của `base.en` nặng ~118MB (vượt giới hạn 100MB/file không-LFS của GitHub), nên phải hạ xuống **`tiny.en`** để decoder q4 chỉ ~87MB. Đánh đổi: độ chính xác thấp hơn `base.en`, nhưng CHẠY ĐÚNG — ưu tiên đúng trước, tối ưu độ chính xác sau (cân nhắc `base.en`/`small.en` lại nếu thư viện vá xong bug q8, hoặc tìm được cấu hình q4 vừa đủ nhỏ). **File model TỰ HOST trong `public/models/onnx-community/whisper-tiny.en/onnx/`** (KHÔNG tải qua CDN Hugging Face — tránh phụ thuộc tốc độ/khả năng truy cập CDN nước ngoài từ VN), `src/lib/whisperSpeech.js` tự chọn theo thiết bị (`navigator.gpu`), dùng chung 1 file decoder q4 cho cả 2 nhánh:
  - **WebGPU** (`encoder_model_fp16.onnx`, ~16.5MB) — chạy trên GPU máy/điện thoại qua trình duyệt, nhanh hơn CPU. Dùng khi trình duyệt hỗ trợ WebGPU (đa số máy tính/điện thoại đời 2023+, Chrome/Edge, Safari 18+/iOS 18+).
  - **WASM/CPU** (`encoder_model_quantized.onnx`, ~10MB) — fallback khi không có WebGPU (Safari cũ, webview trong app Zalo/FB, Android đời thấp...), chậm hơn nhưng chạy được mọi nơi.
  - **`decoder_model_merged_q4.onnx`** (~87MB) — dùng chung cho cả 2 nhánh, dtype q4 (không phải q8) để tránh 2 bug trên.
  Tổng ~117MB trong repo — không tốn phí và không cần Git LFS (GitHub Pages free hosting không có billing, chỉ có soft limit ~1GB repo/~100GB băng thông/tháng; với ~100 học sinh × ~40-55MB/lượt tải đầu ≈ 4-5.5GB tổng, vẫn dưới xa mức khuyến nghị). Trình duyệt tự cache lại bộ đang dùng sau lần tải đầu (Cache Storage của onnxruntime-web) nên chỉ tải 1 LẦN DUY NHẤT mỗi thiết bị, các lượt học sau gần như tức thì — `src/components/ModelPreloader.jsx` được nhúng ở mọi trang chính trong `App.jsx` để bắt đầu tải/preload ngầm (kèm "warm up" biên dịch shader WebGPU) ngay khi mở web, tránh học sinh phải đợi khi bấm mic lần đầu trong bài Speaking. Nếu muốn cải thiện độ chính xác của nhánh dự phòng này thêm, cân nhắc đổi cỡ model nhưng phải kiểm tra dung lượng file từng cái vẫn dưới 100MB (giới hạn cứng của GitHub không LFS) trước khi đổi.
  `src/lib/pronunciationApi.js` (hàm `assessPronunciation()`, trả `{ text, accuracyScore: null }` — không có điểm accuracy chuẩn hoá dù qua Groq hay Whisper local, nơi gọi tự so khớp từ khoá bằng `fuzzyIncludesWord()` trong `src/lib/speech.js`) là nơi quyết định gọi Worker/Groq trước hay rớt thẳng xuống Whisper local, dùng trong `src/components/SceneRunner.jsx` và `src/components/SpeakingMode.jsx`. Không tài khoản/thẻ tín dụng ở cả 2 nhánh. **Lưu ý khác biệt quan trọng:** nhánh Groq audio CÓ được gửi lên server Groq xử lý (khác nhánh Whisper local 100% chạy trong trình duyệt, không server nào nhận audio) — người dùng đã chấp nhận đánh đổi này để lấy độ chính xác/tốc độ tốt hơn. **Không tự ý đổi sang Azure, Git LFS, hay dịch vụ trả phí/cần thẻ tín dụng nào khác khi chưa hỏi lại người dùng.** `SpeechRecognition` (Web Speech API trình duyệt) đã bị loại bỏ hoàn toàn khỏi dự án vì không chạy được trên Safari/iOS.
...
├── worker/                        # Cloudflare Worker proxy Groq Whisper API (deploy độc lập bằng wrangler, xem mục 2), chi tiết ở worker/README.md
├── src/
...
│   │   ├── whisperSpeech.js       # transcribeBlob()/loadTranscriber() — chạy Whisper tiny.en qua @huggingface/transformers, tự chọn WebGPU/WASM, model tự host trong public/models/ (nhánh DỰ PHÒNG)
│   │   ├── pronunciationApi.js    # assessPronunciation() — gọi Worker/Groq trước, rớt xuống whisperSpeech.transcribeBlob() nếu lỗi/chưa cấu hình
...
├── public/models/onnx-community/whisper-tiny.en/   # model Whisper tự host, encoder fp16 (WebGPU) + quantized (WASM) + decoder q4 chung, ~113MB — xem mục 2
...
- **Nhận diện giọng nói (2026-08-21): ưu tiên Cloudflare Worker proxy Groq Whisper API (free, không thẻ tín dụng, chính xác/nhanh hơn), tự động dự phòng sang Whisper `tiny.en` chạy client-side nếu Worker chưa cấu hình/lỗi** — xem mục 2. Nhánh dự phòng dùng `@huggingface/transformers`, tự chọn WebGPU/WASM, model tự host ~117MB không cần Git LFS (không dùng `SpeechRecognition` vì không chạy được trên iOS; từng thử Azure Speech qua Cloudflare Worker nhưng bị chặn vì Azure bắt buộc thẻ tín dụng; cũng từng thử `small.en`/`base.en` nhưng vượt giới hạn 100MB của GitHub hoặc gặp bug thư viện với decoder q8). Cần người dùng tự deploy Worker (`worker/README.md`: tạo tài khoản Groq lấy API key, `wrangler login`, `wrangler secret put`, `wrangler deploy`) và điền `VITE_WORKER_URL` trước khi nhánh Groq hoạt động — Claude không tự làm được các bước tay này. Cần deploy thật lên GitHub Pages để test kỹ trên điện thoại thật (đặc biệt nhánh dự phòng WASM, có thể transcribe chậm hơn máy tính có WebGPU).
