...
- **Nhận diện giọng nói: CHỈ dùng Cloudflare Worker proxy AssemblyAI Speech-to-Text API (batch), KHÔNG còn dự phòng client-side** (xem `src/lib/pronunciationApi.js`). Đây là NGOẠI LỆ BaaS thứ 2 tương tự Firebase/Cloudinary — **AssemblyAI free tier (185 giờ audio batch / 333 giờ streaming) không cần thẻ tín dụng để bắt đầu**, đủ dùng lâu dài cho quy mô ~100 học sinh. Khi hết free tier, trả phí theo giờ dùng thực tế (~$0.21/giờ model Universal-3.5 Pro, ~$0.15/giờ Universal-2) — **người dùng đã CHỦ ĐỘNG chấp nhận trả phí** khi cần, phá vỡ ràng buộc "không tốn phí duy trì"/"không cần thẻ" gốc của dự án (xác nhận 2026-08-22). Worker code ở `worker/` (deploy độc lập bằng `wrangler`, xem `worker/README.md`), cấu hình qua `VITE_WORKER_URL` + secret `ASSEMBLYAI_API_KEY` (`.env.example`, GitHub Actions secret). AssemblyAI xử lý BẤT ĐỒNG BỘ (Worker phải poll kết quả, xem `POLL_INTERVAL_MS`/`MAX_POLL_ATTEMPTS` trong `worker/src/index.js`) nên chậm hơn hẳn Groq trước đây — client timeout **40s** (`FETCH_TIMEOUT_MS` trong `pronunciationApi.js`, nâng từ 25s lên sau load test thật 2026-08-23 cho thấy request thành công có độ trễ tới ~24-25s ở 50-100 học sinh cùng lúc). Nếu `VITE_WORKER_URL` chưa cấu hình hoặc gọi lỗi (mất mạng, AssemblyAI lỗi/timeout...), `assessPronunciation()` tự thử lại tối đa 2 lần rồi ném lỗi thẳng — nơi gọi (`SceneRunner.jsx`, `SpeakingMode.jsx`) báo học sinh thử lại, KHÔNG còn tự chuyển engine khác. **CORS của Worker** (`ALLOWED_ORIGINS`/`isAllowedOrigin()` trong `worker/src/index.js`) cho phép domain thật (`anhngucan.com`, `www.anhngucan.com`, `vanhieu251206.github.io` — thiếu `anhngucan.com` từng làm hỏng ghi âm + xoá học sinh trên web thật từ 2026-09-18 tới 2026-09-25) + cả dải cổng dev `localhost:5173-5179` (Vite tự bump cổng khi cổng trước bận, từng gây lỗi CORS thật khi dev server không chạy đúng cổng 5173 mặc định, phát hiện 2026-08-23) — sửa Worker phải `wrangler deploy` lại mới có hiệu lực, sửa code local không tự áp dụng lên server.
  **Lịch sử đổi engine:** ban đầu dùng Groq Whisper API (`large-v3-turbo`, rẻ hơn AssemblyAI ~4-5 lần, $0.04/giờ) + dự phòng Whisper `tiny.en` chạy client-side (`@huggingface/transformers`, model tự host ~117MB) — dự phòng client-side đã XOÁ HẲN theo yêu cầu người dùng (2026-08-22, xem lịch sử git để khôi phục `whisperSpeech.js`/`ModelPreloader.jsx` nếu cần). Sau đó **CHUYỂN HẲN từ Groq sang AssemblyAI (2026-08-22)** vì Groq tạm khoá nâng cấp gói trả phí (Developer tier báo "nhu cầu quá cao") trong khi giới hạn free 20 request/phút không đủ khi nhiều học sinh luyện Speaking cùng lúc — không tự ý đổi lại Groq hay đổi sang dịch vụ khác khi chưa hỏi lại người dùng, trừ khi Groq mở lại nâng cấp và người dùng chủ động yêu cầu quay lại.
  `src/lib/pronunciationApi.js` (hàm `assessPronunciation()`, trả `{ text, accuracyScore: null }` — không có điểm accuracy chuẩn hoá, nơi gọi tự so khớp từ khoá bằng `fuzzyIncludesWord()` trong `src/lib/speech.js`). Audio CÓ được gửi lên server AssemblyAI xử lý (người dùng đã chấp nhận đánh đổi này để lấy độ chính xác/tốc độ tốt). **Không tự ý đổi sang Azure, Git LFS, hay dịch vụ khác khi chưa hỏi lại người dùng.** `SpeechRecognition` (Web Speech API trình duyệt) đã bị loại bỏ hoàn toàn khỏi dự án vì không chạy được trên Safari/iOS.
...
├── .env.example                   # mẫu biến môi trường (Firebase/Cloudinary/Worker) — copy thành .env (gitignore), điền giá trị thật
├── worker/                        # Cloudflare Worker proxy AssemblyAI Speech-to-Text API (deploy độc lập bằng wrangler, xem mục 2), chi tiết ở worker/README.md
├── src/
...
│   │   ├── pronunciationApi.js    # assessPronunciation() — gọi Worker/AssemblyAI, thử lại vài lần rồi ném lỗi thẳng nếu vẫn lỗi (đã xoá nhánh dự phòng Whisper local, xem mục 2)
...
│   │       ConfirmDialog.jsx  # modal xác nhận tự thiết kế cho khu vực CMS — KHÔNG dùng `window.confirm()` mặc định của trình duyệt (xấu, không theo được theme), dùng `useConfirm()` (2026-08-23)
│   └── pages/
...
- **Nhận diện giọng nói (2026-08-22): CHỈ dùng Cloudflare Worker proxy AssemblyAI Speech-to-Text API (free tier 185 giờ, không thẻ tín dụng để bắt đầu), KHÔNG còn dự phòng client-side** — xem mục 2. Đã xoá hẳn nhánh Whisper local (`@huggingface/transformers`, model tự host ~117MB, `whisperSpeech.js`, `ModelPreloader.jsx`) theo yêu cầu người dùng để đơn giản hoá. Sau đó đã CHUYỂN TỪ GROQ SANG ASSEMBLYAI (cùng ngày) vì Groq tạm khoá nâng cấp gói trả phí trong khi free tier 20 request/phút không đủ cho lớp đông — người dùng đã xác nhận chấp nhận trả phí khi hết free tier AssemblyAI (~$0.21/giờ), phá vỡ ràng buộc "không tốn phí"/"không cần thẻ" gốc của dự án (không dùng `SpeechRecognition` vì không chạy được trên iOS; từng thử Azure Speech qua Cloudflare Worker nhưng bị chặn vì Azure bắt buộc thẻ tín dụng). Cần người dùng tự deploy Worker (`worker/README.md`: tạo tài khoản AssemblyAI lấy API key, `wrangler login`, `wrangler secret put ASSEMBLYAI_API_KEY`, `wrangler deploy`) và điền `VITE_WORKER_URL` trước khi tính năng ghi âm hoạt động — Claude không tự làm được các bước tay này. Cần deploy thật lên GitHub Pages để test kỹ trên điện thoại thật, đặc biệt độ trễ do AssemblyAI xử lý bất đồng bộ (chậm hơn Groq).
...
- **Cập nhật 2026-09-20 (thay thế mô tả cũ về mật khẩu chung/mật khẩu theo bộ đề/guest ở trên):**
  ...
  - **Kết quả bài làm chỉ lưu tới 48h SAU HẠN CHÓT của lần mở bài rồi bị XOÁ HẲN (chốt 2026-09-25, áp dụng cả admin — để không vượt hạn mức lưu trữ free).** Mở bài BẮT BUỘC có hạn chót. `testResults` lưu `openingId` + `purgeAfter`; gia hạn bài thì mốc xoá lùi theo. Không có server chạy định kỳ nên `purgeExpiredResults()` (`lib/testResults.js`) chạy mỗi khi admin/giáo viên mở khu vực quản trị — xoá kết quả + `speakingSessions`/`events` đi kèm. `attempts` (đếm lượt) và `speechLogs` KHÔNG bị xoá.
  - **Phiếu chấm bài PDF (2026-09-25, `lib/resultSheetPdf.js`):** chỉ tải được từ lúc HẾT HẠN NỘP tới khi kết quả bị xoá (hạn chót + 48h) — nút "⬇ Phiếu chấm" cả lớp ở trang Mở bài (bảng điểm + mỗi lượt nộp 1 phiếu) và "⬇ Phiếu" từng em ở trang Kết quả. Admin + mọi giáo viên (giáo viên phụ theo lớp được giao). Thiết kế in trắng đen, chữ vector font Be Vietnam Pro (`public/assets/fonts`, OFL) + jspdf-autotable. Đóng bài (xoá lần mở) thì mất nút tải vì không còn hạn chót để đối chiếu.
  - Đáp án nhiều cách viết cho câu trả lời ngắn Reading: ngăn bằng dấu `|`.
  - **Siết bảo mật trước launch (2026-09-25):** `testResults`/`speakingSessions`/`events`/`speechLogs` bắt buộc đăng nhập + ghi đúng `uid` của mình (session/log giờ lưu `uid`), chỉ staff đọc; `attempts` học sinh chỉ đọc lượt của mình (theo id doc). Worker `/transcribe` bắt buộc Firebase ID token + giới hạn 20 lượt/phút/tài khoản. CSP dạng `<meta>` chỉ gắn lúc build (`cspPlugin` trong `vite.config.js`) — thêm dịch vụ ngoài mới (script/iframe/API) phải thêm domain vào đó. Có trang `privacy` (Chính sách bảo mật), 404 (`NotFoundPage.jsx` + `public/404.html`), `robots.txt`, `sitemap.xml`, og-image, tiêu đề tab theo trang (`PAGE_TITLES` trong `App.jsx`), khoá tạm form đăng nhập khi sai nhiều lần. `lessons` (+ subcollection, có đáp án) chỉ tài khoản đã đăng nhập đọc được. Tuỳ chọn miễn phí bật bằng biến môi trường: Google Analytics (`VITE_GA_MEASUREMENT_ID`, `lib/analytics.js`, chỉ gửi tên trang) và Firebase App Check reCAPTCHA v3 (`VITE_RECAPTCHA_SITE_KEY`, `lib/firebase.js`). Rules đã test bằng emulator (firebase-tools 13 vì máy có Java 17; bản 15 cần Java 21).
  - **Chấm bài phía máy chủ + giấu đáp án (2026-09-25, "sửa tận gốc"):** đáp án Reading/IELTS Reading+Listening/KET-PET Vocabulary+Practice Test/Listening luyện đề (Part 1-3, Movers Part 3-4) tách khỏi đề sang collection `answerKeys` (chỉ admin/giáo viên/tester đọc) — `lib/grading/answerKeys.js` (tách/ghép theo đường dẫn, lưu chuỗi JSON), `lib/answerStore.js` (CMS lưu = tách, đọc = ghép nếu `canReadAnswers`). Học sinh nộp bài qua Worker `POST /test/start` + `/test/submit` (`worker/src/submit.js`, `lib/testSubmit.js` hook `useTestSubmission`): Worker kiểm tra lớp/lần mở bài/hạn chót/số lượt/thời gian theo giờ máy chủ, tự chấm bằng code chấm DÙNG CHUNG `src/lib/grading/*` (thuần JS — Worker import thẳng, KHÔNG được import React/Firebase/import.meta vào đây), ghi `testResults` + cộng `attempts` (rules chặn trình duyệt ghi 2 collection này). Speaking, Dictation, Listening tô màu (Part 4 Starters/Part 5 Movers — chấm bằng điểm ảnh canvas) vẫn chấm ở trình duyệt (phản hồi từng câu) nhưng vẫn nộp qua Worker (kẹp điểm, kiểm soát lượt/hạn). Đề cũ chuyển bằng nút admin Tổng quan → "Tách đáp án bài cũ" (`lib/answerMigration.js`). Dạng "sắp xếp thứ tự" KET/PET: đề công khai lưu thứ tự đã xáo, vị trí đúng `correctPos` nằm trong khoá. Kiểm thử: emulator (firebase-tools 13) cho rules + Worker (chặn fetch trỏ Firestore REST về emulator).
