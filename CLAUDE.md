...
- **Nhận diện giọng nói: dùng DUY NHẤT Azure AI Speech Pronunciation Assessment qua Cloudflare Worker proxy** (đổi 2026-08-17, bỏ hẳn Whisper WASM fallback theo yêu cầu người dùng). Lý do cần Worker: không thể gọi Azure Speech thẳng từ trình duyệt vì sẽ lộ API key qua DevTools — Worker giữ key bí mật (`wrangler secret put`, không nằm trong bundle JS). Đây là dịch vụ BaaS (không tự vận hành server), giống tinh thần ngoại lệ Firebase — **NGOẠI LỆ đã xác nhận (2026-08-15): Cloudflare Worker (free tier, không cần thẻ tín dụng)**. **Phạm vi CHỈ giới hạn ở việc proxy này** — không tự ý biến Worker thành server tổng quát cho logic khác khi chưa hỏi lại người dùng (cùng tinh thần lệnh cấm Cloud Functions ở trên). Code Worker nằm ở thư mục riêng `worker/` (deploy độc lập bằng `wrangler deploy`, không qua GitHub Actions của site chính — xem `worker/README.md`). Frontend gọi qua `src/lib/pronunciationApi.js` (hàm `assessPronunciation()`), dùng trong `src/components/SceneRunner.jsx` và `src/components/SpeakingMode.jsx`. **KHÔNG còn fallback nào** — nếu Worker/Azure lỗi hoặc chưa cấu hình `VITE_WORKER_URL`, học sinh sẽ thấy thông báo lỗi thay vì rớt xuống engine khác (đánh đổi đã được người dùng xác nhận chấp nhận). Rủi ro chấp nhận: endpoint `/pronunciation` không xác thực (học sinh là khách ẩn danh, không có tài khoản để xác thực) — giảm nhẹ bằng CORS giới hạn domain + rate-limit theo IP trong Worker, người dùng tự theo dõi usage trên Azure Portal và đổi key nếu bị lạm dụng (đánh đổi tương tự cơ chế mật khẩu hash ở trên). Azure Speech dùng tier F0 (free, 5 giờ/tháng, hết hạn mức thì bị từ chối request chứ không tự tính phí). `SpeechRecognition` (Web Speech API trình duyệt) và Whisper WASM (`@xenova/transformers`) đều đã bị loại bỏ hoàn toàn khỏi dự án — `SpeechRecognition` vì không chạy được trên Safari/iOS, Whisper vì chỉ transcribe text (không chấm accuracy/fluency như Azure) và người dùng chủ động chọn bỏ để đơn giản hoá.
...
├── worker/                        # Cloudflare Worker riêng (deploy độc lập bằng wrangler, xem mục 2 — ngoại lệ Azure Speech), chi tiết ở worker/README.md
├── src/
...
│   │   ├── speech.js              # speak() TTS (rate chậm + timeout dự phòng), normalize(), isRecordingSupported()
│   │   ├── pronunciationApi.js    # gọi Cloudflare Worker chấm phát âm qua Azure Speech (assessPronunciation) — engine nhận diện giọng nói DUY NHẤT, không fallback
