...
- **Nhận diện giọng nói: đã đổi từ `SpeechRecognition` sang Whisper-tiny chạy WASM ngay trong trình duyệt** (`@xenova/transformers`, xem `src/lib/whisperSpeech.js`) — lý do: `SpeechRecognition` không chạy được trên Safari/iOS (mọi trình duyệt iOS đều dùng chung engine WebKit theo quy định Apple). Whisper WASM chạy client-side, miễn phí, nhất quán trên mọi nền tảng. Model tự tải qua CDN Hugging Face lần đầu vào web (xem `ModelPreloader.jsx`, hiện card loading góc màn hình), trình duyệt tự cache lại. **Vẫn là engine mặc định/fallback** — xem ngoại lệ Cloudflare Worker bên dưới cho engine nâng cao (không bắt buộc).
- **NGOẠI LỆ đã xác nhận (2026-08-15): Cloudflare Worker (free tier, không cần thẻ tín dụng) để proxy Azure AI Speech Pronunciation Assessment**, chấm phát âm chính xác hơn Whisper (Whisper chỉ transcribe text, không chấm accuracy/fluency). Lý do cần Worker: không thể gọi Azure Speech thẳng từ trình duyệt vì sẽ lộ API key qua DevTools — Worker giữ key bí mật (`wrangler secret put`, không nằm trong bundle JS). Đây là dịch vụ BaaS (không tự vận hành server), giống tinh thần ngoại lệ Firebase. **Phạm vi CHỈ giới hạn ở việc proxy này** — không tự ý biến Worker thành server tổng quát cho logic khác khi chưa hỏi lại người dùng (cùng tinh thần lệnh cấm Cloud Functions ở trên). Code Worker nằm ở thư mục riêng `worker/` (deploy độc lập bằng `wrangler deploy`, không qua GitHub Actions của site chính — xem `worker/README.md`). Frontend gọi qua `src/lib/pronunciationApi.js` (hàm `assessPronunciation()`), dùng trong `src/components/SceneRunner.jsx`: **thử Azure trước, lỗi hoặc chưa cấu hình `VITE_WORKER_URL` thì tự rơi về Whisper WASM** — không bắt buộc, không phá app nếu Worker/Azure gặp sự cố. Rủi ro chấp nhận: endpoint `/pronunciation` không xác thực (học sinh là khách ẩn danh, không có tài khoản để xác thực) — giảm nhẹ bằng CORS giới hạn domain + rate-limit theo IP trong Worker, người dùng tự theo dõi usage trên Azure Portal và đổi key nếu bị lạm dụng (đánh đổi tương tự cơ chế mật khẩu hash ở trên). Azure Speech dùng tier F0 (free, 5 giờ/tháng, hết hạn mức thì bị từ chối request chứ không tự tính phí).
...
│   │   ├── whisperSpeech.js       # nhận diện giọng nói Whisper WASM (loadTranscriber, transcribeBlob có timeout) — engine mặc định/fallback
│   │   ├── pronunciationApi.js    # gọi Cloudflare Worker chấm phát âm qua Azure Speech (assessPronunciation) — engine nâng cao, không bắt buộc
